...
public class KisStockMasterSyncRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        collect(stocks, KisStockMasterMarket.KOSPI, properties.kospiUrl());
        collect(stocks, KisStockMasterMarket.KOSDAQ, properties.kosdaqUrl());
        collect(stocks, KisStockMasterMarket.KONEX, properties.konexUrl());
        if (stocks.isEmpty()) {
            log.warn("KIS stock master sync skipped because no rows were downloaded");
            return;
        }

        repository.upsertAll(stocks);
        log.info("KIS stock master synced: count={}", stocks.size());
    }

    private void collect(List<StockSummary> stocks, KisStockMasterMarket market, String url) {
        try {
            List<StockSummary> marketStocks = client.fetch(market, url);
            stocks.addAll(marketStocks);
