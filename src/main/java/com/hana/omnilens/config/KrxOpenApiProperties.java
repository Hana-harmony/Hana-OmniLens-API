...
@ConfigurationProperties(prefix = "omnilens.providers.krx-open-api")
public record KrxOpenApiProperties(
    ...
    public String requiredAuthKey() {
        if (!StringUtils.hasText(authKey)) {
            throw new IllegalStateException("omnilens.providers.krx-open-api.auth-key is not configured");
        }
        return authKey;
    }
}
