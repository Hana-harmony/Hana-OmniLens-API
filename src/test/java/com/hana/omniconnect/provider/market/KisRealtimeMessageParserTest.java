...
class KisRealtimeMessageParserTest {
    ...
    void parseTradeTickMapsKisTradePayload() {
        ...
        assertThat(tick.orElseThrow().singlePriceTradingCode()).isEmpty();
        assertThat(tick.orElseThrow().tradingHaltCode()).isEqualTo("HALT");
    }

    @Test
    void parseMarketStatusMapsCircuitBreakerNotification() {
        Optional<KisRealtimeMarketStatus> status = parser.parseMarketStatus(kisFrame(
                KisRealtimeTransaction.MARKET_STATUS,
                "005930^Y^서킷브레이커 발동^2^2^0^0^0^0^0^KRX"));

        assertThat(status).isPresent();
        assertThat(status.orElseThrow().stockCode()).isEqualTo("005930");
        assertThat(status.orElseThrow().tradingHalted()).isTrue();
        assertThat(status.orElseThrow().circuitBreakerActive()).isTrue();
    }
