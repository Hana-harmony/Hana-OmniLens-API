...
class HannahAiKoreanTranslationClientTest {
    ...
    @Test
    void translateParsesJsonBodyEvenWhenContentTypeIsOctetStream() {
        RestClient.Builder builder = RestClient.builder();
        MockRestServiceServer server = MockRestServiceServer.bindTo(builder).build();
        HannahAiKoreanTranslationClient client = new HannahAiKoreanTranslationClient(
                builder.baseUrl("http://localhost:8000").build(),
                ...
                .andRespond(withSuccess("""
                        {
                          ...
                          "message": "OK",
                          "data": {
                            "translated_text": "KOSPI rebounded.",
                            "provider": "local-open-source-qwen3-translation",
                            "model_version": "local-llm:Qwen3-4B-GGUF-Q4",
                            "status": "TRANSLATED",
                            "prompt_version": "ko-en-qwen3-financial-translation-v2",
                            "quality_flags": []
                          }
                        }
        ...
        assertThat(response.translatedText()).isEqualTo("KOSPI rebounded.");
        assertThat(response.status()).isEqualTo("TRANSLATED");
        server.verify();
    }
}
