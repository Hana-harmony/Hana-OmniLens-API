...
- `Hana Montana AI(KF-DeBERTa + K-FNSPID)`가 종목 연관성·이벤트·감성을 분류하고, 의미 중요도와 K-FNSPID v4 뉴스·공시 출처별 사후 가격반응 등급을 독립 신호로 반환한다. 요청 출처와 일치하는 품질 gate 통과 전문가만 사용하며 가격반응은 의미 중요도를 덮어쓰지 않는다.
...
- 관리자 계정은 제한된 운영 DB 절차로만 생성·초기화하며 애플리케이션 시작 환경변수로 관리자 비밀번호를 주입하지 않는다. 비밀번호 초기화 시 세션 버전을 올리고 다음 로그인에서 변경을 강제한다.
- 포털 비밀번호는 12~128자이며 API 키 원문은 암호화 보관한다. 활성 키의 소유 회원은 현재 비밀번호 재확인 후 횟수 제한 없이 다시 조회할 수 있고, 관리자와 다른 회원에게는 원문을 제공하지 않는다. 취소·승인·반려·재발급·폐기는 상태 전이와 RBAC로 통제한다.
...
로컬 시크릿은 gitignore된 `src/main/resources/application-local.yml`에만 둔다. 관리자 비밀번호는 설정 파일이나 환경변수에 저장하지 않는다.
