...
## 운영
- OmniConnect FE 운영 주소는 `https://hanaomni.cloud`, API 운영 주소는 `https://api.hanaomni.cloud`이다.
- `main` push 시 GitHub Actions의 `deploy-prod` job이 실행된다.
- CI는 ARM64 API 이미지를 빌드해 GHCR에 push한다.
- OCI `VM.Standard.A1.Flex` 한 대에서 PostgreSQL, Redis, OmniConnect API를 컨테이너로 운영한다.
- PostgreSQL과 Redis는 `hana-omni-connect-internal` 네트워크에서만 접근하며 호스트 포트를 공개하지 않는다.
...
- HTTPS server block은 Ubuntu 24.04 기본 nginx 1.24와 호환되는 `listen 443 ssl http2;` 문법을 사용한다.
...
- `OMNI_CONNECT_PORTAL_SESSION_SIGNING_KEY`
- `OMNI_CONNECT_PORTAL_API_KEY_ENCRYPTION_KEY`
- `OMNI_CONNECT_TERM_ANALYTICS_HASH_SALT`
- `HANNAH_AI_MAINTENANCE_TOKEN`

OCI 최초 배포에서 `/opt/hana-runtime/root-secret`을 256-bit로 생성하고 용도별 HMAC 라벨로 서로 다른 값을 파생한다. Hana-Omni-Connect-API와 Hannah-Montana-AI는 같은 OCI 호스트 루트와 `hana/ai/maintenance-auth/v1` 라벨을 사용하므로 별도 공유 Secret 없이 동일한 유지보수 토큰을 얻는다. 루트 파일은 재배포 때 덮어쓰지 않고 `600` 권한으로 유지한다. 루트키가 유실되면 기존 포털 API 키 암호문을 복호화할 수 없으므로 OCI 암호화 볼륨 백업에 반드시 포함한다.

DB와 Redis 연결 정보는 배포 계약으로 고정한다.

Redis 애플리케이션 계정은 위험 명령군을 차단한 상태에서 Spring Boot 상태 점검에 필요한 `INFO` 명령만 예외적으로 허용한다. `@dangerous` 명령군 전체를 허용하지 않는다.

```text
DB_URL=jdbc:postgresql://hana-omni-connect-postgres:5432/omni-connect
DB_USERNAME=omni_connect_app
REDIS_HOST=hana-omni-connect-redis
REDIS_PORT=6379
...
- `OMNI_CONNECT_TERM_ANALYTICS_HASH_SALT`: 한국 금융 용어 클릭 로그의 사용자/세션 식별자 salted hash에 사용하며 운영에서는 호스트 루트키에서 자동 파생한다.
...
- `HANNAH_AI_MAINTENANCE_TOKEN`: Hannah 재학습 endpoint 보호용 내부 토큰이며 OmniConnect API와 AI가 같은 OCI 호스트 루트키에서 자동 파생한다.
...
- `KIS_ACCOUNT_NUMBER`, `KIS_APP_KEY`, `KIS_APP_SECRET`: 일반 종목 REST·WebSocket용 모의투자 자격증명이다. 로컬 Compose에서는 대응하는 `OMNI_CONNECT_PROVIDERS_KIS_*` 변수와 모의 endpoint를 사용한다.
- `KIS_REAL_ACCOUNT_NUMBER`, `KIS_REAL_APP_KEY`, `KIS_REAL_APP_SECRET`: 실전 지수 WebSocket 전용 자격증명이다. 로컬 Compose에서는 대응하는 `OMNI_CONNECT_PROVIDERS_REAL_KIS_*` 변수와 실전 endpoint를 사용한다. 두 계정의 자격증명과 endpoint를 교차 사용하지 않는다.
...
- `OMNI_CONNECT_SECURITY_SIGNATURE_NONCE_STORE_MODE`: 서명 nonce 저장소. 애플리케이션과 운영 기본값은 `redis`, 격리된 자동 테스트는 `in-memory`를 사용할 수 있다.
- `OMNI_CONNECT_SECURITY_SIGNATURE_MAX_NONCES`: 인메모리 nonce 저장 최대 개수. 기본값은 `10000`이다.
- `OMNI_CONNECT_MTLS_ENABLED`: 보호 API client certificate 필수 검증 활성화 여부. 기본값은 `false`이다.
...
- `ALERT_SCHEDULER_FIXED_DELAY_MS`: 주기 수집 간격. 기본값은 `600000`이다.
...
- `NAVER_NEWS_QUERY_CACHE_TTL`: 같은 검색어 결과를 공유하는 시간. 기본값은 `10m`이다.
- `NAVER_NEWS_QUERY_CACHE_MAX_ENTRIES`: 네이버 뉴스 검색 cache 최대 항목 수. 기본값은 `1024`이다.
- `NAVER_NEWS_DAILY_REQUEST_LIMIT`: 네이버 애플리케이션 일일 호출 한도. 기본값은 `25000`이다.
- `NAVER_NEWS_DAILY_REQUEST_RESERVE`: 장애 대응과 수동 조회를 위해 자동 수집에서 사용하지 않는 일일 여유분. 기본값은 `5000`이다.
...
OMNI_CONNECT_MTLS_ENABLED=true
...
배포 파일과 `/opt/hana-omni-connect-api` 디렉터리는 CI/CD가 생성한다.
