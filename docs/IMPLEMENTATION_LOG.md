...
- 뉴스·공시 상세 조회에서 전체 영문 본문이 없거나 What/Why/Impact 요약 형태이면 Hannah 전문 번역을 보강한 뒤 반환한다.
- OmniConnect와 Hannah의 이중 본문 분할을 제거하고 미번역 큐를 실행당 최대 10건씩 연속 처리한다.
...
## 2026-07-21 · 후속 전문 번역 중복 제거

- 뉴스 초기 적재에서 이미 저장한 종목 연결, 감성·중요도·시장영향 분류, What/Why/Impact 3줄 요약과 제목 번역을 후속 전문 번역에서 재사용한다.
- 후속 작업은 비어 있는 `translatedContent`만 Hannah AI 번역 API로 채우며, 기존 분석·요약 결과를 다시 계산하지 않는다.
- 번역이 빈 본문을 반환하거나 실패하면 원문 보존 상태를 유지하고 대기 로그를 남긴다.
- Redis `SET NX` 유예 키로 실패 기사를 6시간 뒤 재시도하고, 즉시 다음 대기 기사로 넘어가 특정 기사가 전체 대기열을 막지 않게 한다.

## 2026-07-20 · 뉴스·공시 초기 적재와 전체 본문 번역 분리

- 인기 종목과 외국인 취득한도 제한 종목의 주기 수집을 전체 본문 Qwen 번역 완료에서 분리했다.
- 수집 단계는 종목 연결·감성·중요도·시장영향·영문 What/Why/Impact와 원문을 먼저 저장하고 `ORIGINAL_TEXT_ONLY`로 노출한다.
- 전용 2-worker 보강 큐가 종목 알림과 시장뉴스의 전체 번역을 자동 완성하고 성공한 레코드를 `FULL_TEXT`로 전환한다.
- 한국 증시 시장뉴스도 영문 분석이 완성되면 저장·목록·상세에 노출하며 상세 조회에서 장시간 전체 번역을 실행하지 않는다.
...
# 2026-07-21 · 전문 번역 상세 조회 비동기화

- 장문 전문 번역이 reverse proxy 제한 시간을 넘어 상세 GET이 504가 되던 경로를 요청-응답 수명에서 분리했다.
- 상세 조회는 저장 이벤트를 즉시 반환하고 별도 bounded executor에서 번역하며, 일반 적체 worker는 한 슬롯만 사용한다.
- 거래소 FE가 빈 `translatedContent`를 재조회할 수 있도록 기존 응답 계약은 유지했다.
