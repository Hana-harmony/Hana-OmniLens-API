...
| KRX 외국인 보유량 snapshot cache | `Hana-OmniLens-API` | KRX Data Marketplace 로그인 기반 `MDCSTAT03702` 응답으로 외국인 보유수량, 보유율, 한도수량, 한도소진율 저장 | Done |
| 주문 전 외국인 한도소진율 예측 boundary | `Hana-OmniLens-API` | KRX 외국인보유량 snapshot과 일별 시계열로 금일 한도 도달 가능성 min/base/max 산출 | Done |
| 당일 외국인 한도 사전 고지 engine | `Hana-OmniLens-API`, Predict Engine | 외국인 취득 수량 시계열 기반 한도 도달 가능성을 경고 신호로 제공한다. 주문수량과 장중 거래량은 예측식에 반영하지 않고 외국인 한도만으로 주문을 차단하지 않는다. | Done |
...
| 기능 | 담당 | 내용 | 상태 |
| --- | --- | --- | --- |
| 뉴스 수집 | `Hana-OmniLens-API` | Naver News Search API로 한국어 뉴스 제목, snippet, 원문 링크, 언론사, 발행시각을 발견하고 사용 허가된 원문 URL에서 전문과 대표 이미지를 수집 | Done |
| 공시 수집 | `Hana-OmniLens-API` | OpenDART 공시 목록과 접수번호 기반 document 원문을 수집해 공시 전문을 정제 | Done |
...
| 번역 | `Hana-OmniLens-API` | DeepL adapter로 제목, What/Why/Impact 요약, 전문을 chunk 단위로 영어 번역하고 장애 시 원문 fallback | Done |
...
| v2 보강 기능 | 담당 | 내용 | 상태 |
| --- | --- | --- | --- |
| 전체 종목 수집 스케줄러 | `Hana-OmniLens-API` | `stock_master` 전체 universe를 shard로 나누어 Naver News Search 발견 데이터와 OpenDART 공시를 주기 수집 | Done for watchlist path, full-universe shard 확장 지속 |
| 전문·이미지 수집 계약 | `Hana-OmniLens-API` | 사용 허가된 Naver 원문 링크에서 기사 전문과 대표 이미지 URL을 추출하고 OpenDART document 원문을 정제 | Done |
| 영속 뉴스 이벤트 저장소 | `Hana-OmniLens-API` | alert event 원문 JSON과 stock/source/duplicate/cluster metadata를 DB에 저장 | Done |
| 전문 기반 AI 분석 | `Hannah-Montana-AI` | 제목/snippet/전문을 함께 입력받아 종목 매핑, 이벤트 태그, 감성, 중요도, What/Why/Impact 3줄 요약, 중복 키를 생성. 실제 원문 전문 데이터셋으로 재학습하고 release/service readiness를 통과 | Done |
| 전문 번역 | `Hana-OmniLens-API` | DeepL adapter로 제목, What/Why/Impact 요약, 전문을 chunk 단위로 번역하고 원문 fallback/cache를 제공 | Done |
...
| Hannah-Montana-AI 분석 API | Done: 제목/snippet 및 권리 안전 전문 입력 분석, What/Why/Impact 3줄 요약, full-content training release/service readiness pass |
...
외부 provider credential은 저장소에 두지 않으며 `application-local.yml`, 배포 환경 변수, 또는 Secret Manager로만 주입한다. 따라서 위 `Done`은 KIS REST/WebSocket, KRX Data Marketplace 로그인 기반 외국인 보유량 snapshot, KRX Open API history, FX refresh, Naver/OpenDART 수집, DeepL 번역 adapter와 계약 테스트가 구현되어 있다는 의미이고, KRX는 서비스 이용 신청이 완료된 인증키로 smoke validation을 별도로 확인한다. KIS 실시간 체결가·호가 WebSocket에는 외국인 보유수량, 보유율, 한도소진율 필드가 없으므로 외국인 한도 정보는 KRX snapshot과 cache로 공급한다.
