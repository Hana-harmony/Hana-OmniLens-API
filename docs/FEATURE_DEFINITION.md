...
| 주문 전 외국인 한도소진율 예측 boundary | `Hana-OmniLens-API` | KRX 외국인보유량 cache, 요청 수량, KIS 실시간 체결 누적 거래량으로 BUY 예상 한도소진율 min/base/max 산출 | Done |
| 정교한 당일 외국인 지분율 예측 engine | `Hana-OmniLens-API`, Predict Engine | 주문 영향도와 당일 거래량 기반 불확실성을 반영한 예상 지분율 min/base/max 산출 | Done |
...
| AI 분석 | `Hannah-Montana-AI` | 종목 매핑, 이벤트 태그, 감성, 중요도, 요약, 중복 제거 키 생성, 금융 용어 normalization과 번역 품질 플래그 생성 | Implemented, audited gold coverage pending |
...
| 조세조약 케이스 판정 | `Hana-OmniLens-API` | 한국·홍콩 상장주식 장내거래, 25% 미만 지분율, 필수 서류 검증 기반 CASE_01 분류 | Done |
...
| Hannah-Montana-AI 분석 API | Implemented, audited gold coverage pending |
| KIS/KRX/FX/Naver/OpenDART/Papago/DeepL 실연동 | Done for adapter/contracts and env injection, production credential smoke validation은 배포 단계에서 수행 |
...
| 세무/OCR/환급 선지급 구현 | Hana tax case classification/status sync Done, AI OCR/위변조 1차 검증 Done, 법무 최종판정/실제 지급은 Planned |

외부 provider credential은 저장소에 두지 않으며 `application-local.yml`, 배포 환경 변수, 또는 Secret Manager로만 주입한다. 따라서 위 `Done`은 KIS REST/WebSocket, KRX history, FX refresh, Naver/OpenDART 수집, Papago/DeepL 번역 adapter와 계약 테스트가 구현되어 있다는 의미이고, 실제 운영 credential을 사용한 smoke validation은 배포 체크리스트에서 별도로 확인한다.
