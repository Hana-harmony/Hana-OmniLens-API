...
| 실시간 시세 화면 | `Stock-exchange-FE` | 전체/시장별/watchlist/보유종목 시세 목록과 종목 상세 표시 | Partial |
...
| 기능 | 담당 | 내용 | 상태 |
| --- | --- | --- | --- |
| 환율 수집과 캐시 | `Hana-OmniLens-API` | 한국수출입은행 또는 승인된 FX provider의 실시간/최신 환율 수집 | Done |
...
| 원화/현지통화 표시 | `Stock-exchange-FE` | 현재가 KRW, 현지통화 환산 가격, 적용 환율 기준시각 표시 | Partial |
...
| 과거 시세 차트 | `Stock-exchange-FE` | Flutter 앱에서 일봉/기간 차트 표시 | Partial |
...
| KRX 외국인 보유율 캐시 | `Hana-OmniLens-API` | 전일 외국인 보유수량, 보유율, 한도소진율 저장 | Done |
| 주문 전 외국인 한도소진율 예측 boundary | `Hana-OmniLens-API` | KRX 외국인보유량 cache와 요청 수량으로 BUY 예상 한도소진율 산출 | Done |
| 정교한 당일 외국인 지분율 예측 engine | `Hana-OmniLens-API`, Predict Engine | 당일 예상 지분율 min/max 산출 | Planned |
...
| 주문/종목 화면 표시 | `Stock-exchange-FE` | 외국인 한도 게이지, VI 배지, 상·하한가 배지, 주문 제한 팝업 | Partial |
...
| AI 분석 | `Hannah-Montana-AI` | 종목 매핑, 이벤트 태그, 감성, 중요도, 요약, 중복 제거 키 생성, 금융 용어 normalization과 번역 품질 플래그 생성 | Partial |
| 번역 | `Hana-OmniLens-API` | DeepL 우선, Papago fallback adapter로 제목을 영어로 번역하고 장애 시 원문 fallback | Done |
...
| K-News와 알림함 UI | `Stock-exchange-FE` | 번역 제목, 요약, 태그, 감성, 중요도, 원문 링크, 통합 알림함 표시 | Partial |

뉴스·공시 흐름:

`Naver/OpenDART -> Hana-OmniLens-API -> Hannah-Montana-AI analysis/translation quality metadata -> Papago/DeepL translation -> Hana WebSocket alert -> Stock-exchange-BE matching/push -> Flutter iOS/Android 앱`
...
| 기능 | 담당 | 내용 | 상태 |
| --- | --- | --- | --- |
| 세무 서류 업로드 접수 | `Stock-exchange-BE`, `Stock-exchange-FE` | 거주자증명서, 제한세율신청서 PDF/JPG 업로드 | Partial |
...
| 세무 화면 | `Stock-exchange-FE` | 서류 업로드, 상태 타임라인, 정산 상세, 환급 신청, 선지급 완료/리스크 고지 | Partial |
...
| Hana orderability boundary API | 외국인 한도소진율 예측, 상·하한가, VI, 단일가, 거래정지 상태 Done |
| Hana market quote WebSocket stream | KIS tick 기반 raw WebSocket Done |
| Hana alert WebSocket 송신 | Done |
| Hana API key 인증 | Partial |
| Hannah-Montana-AI 분석 API | Partial |
| KIS/KRX/FX/Naver/OpenDART/Papago/DeepL 실연동 | Partial |
| Stock-exchange-BE 실제 서버 구현 | Spring Boot 기반 Partial, 외부 push provider 실발송만 Planned |
| Stock-exchange-FE Flutter 앱 구현 | Flutter iOS/Android 앱 기반 Partial |
| 세무/OCR/환급 선지급 구현 | Hana tax status sync Done, OCR/법무 판정/실제 지급은 Planned |
