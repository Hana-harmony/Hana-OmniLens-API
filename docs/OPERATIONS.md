...
- 종목별 수집은 OpenDART 공시를 먼저 처리해 Qwen 뉴스 작업이 공시 복구를 지연시키지 않게 한다. OpenDART `document.xml`이 ZIP 대신 `status/message` 오류 envelope를 반환하면 저장하지 않고 다음 스케줄에서 재시도한다. 한 종목의 원문 확인은 실행당 최대 20건으로 제한해 문서 준비 지연 시 API 호출이 폭주하지 않게 한다. `V25__remove_invalid_opendart_document_payloads.sql`은 과거 `014 파일이 존재하지 않습니다` 응답을 본문으로 저장한 공시만 제거한다.
...
- 전문 미완료 레코드는 API 목록·상세·트렌딩에서 제외된다. 클라이언트는 상세 GET을 재조회하지 않으며, 운영자는 `FULL_TEXT`·`translatedContent`·번역 소요 시간 로그로 적체와 Qwen 추론 지연을 확인한다.
