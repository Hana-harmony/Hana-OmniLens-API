...
| 기능 | 책임 | 상태 |
| --- | --- | --- |
| KIS 현재가 REST, 실시간 체결가·호가 WebSocket 수집 | Hana-OmniLens-API | Planned |
| KIS 실시간 시세 캐시와 전체/다건 종목 quote snapshot API | Hana-OmniLens-API | Planned |
| 협력사용 market quote WebSocket stream | Hana-OmniLens-API | Planned |
| 실시간 또는 최신 환율 수집과 환율 캐시 | Hana-OmniLens-API | Planned |
| KRW 가격과 현지통화 환산 가격을 함께 포함한 quote payload | Hana-OmniLens-API | Planned |
| KRX 모든 국내 주식 과거 시세 수집과 DB 저장 | Hana-OmniLens-API | Planned |
| 과거 시세 chart REST API | Hana-OmniLens-API | Planned |
| KIS 종목 마스터 파일 파싱, 종목 마스터 DB 적재 | Hana-OmniLens-API | Planned |
| KRX 전일 외국인 보유수량, 보유율, 한도소진율 캐시 | Hana-OmniLens-API | Planned |
| 당일 외국인 보유율/한도소진율 예측 boundary | Hana-OmniLens-API, Predict Engine | Planned |
| VI 발동, 단일가 매매, 상·하한가 상태 감지 | Hana-OmniLens-API | Planned |
| 종목 상세/주문 상태 JSON API | Hana-OmniLens-API | Partial |
| 모의 매수·매도 기준 가격과 주문 가능 여부 판단 | Stock-exchange-BE | Planned |
| 아이디/비밀번호 회원가입, mock USD 계좌 생성, 달러 충전 | Stock-exchange-BE | Planned |
| KIS 모의투자 API가 아닌 자체 mock ledger 기반 가짜 매수·매도 | Stock-exchange-BE | Planned |
| 매도 실현손익을 세무 환급/선지급 입력 데이터로 연결 | Stock-exchange-BE | Planned |
...
| 기능 | 책임 | 상태 |
| --- | --- | --- |
| Naver News Search 수집 | Hana-OmniLens-API | Planned |
| OpenDART 공시 수집 | Hana-OmniLens-API | Planned |
| 종목 매핑, 이벤트 분류, 감성, 중요도, 중복 제거 | Hannah-Montana-AI | Partial |
| Papago/DeepL 전문 번역 어댑터 | Hana-OmniLens-API | Planned |
| 분석·번역 완료 이벤트 WebSocket 송신 | Hana-OmniLens-API | Partial |
| 이벤트 수신, 저장, 보유/관심종목 대상자 매칭, 푸시 발송 | Stock-exchange-BE | Planned |
...
| 기능 | 책임 | 상태 |
| --- | --- | --- |
| 거주자증명서, 제한세율신청서, 거래원장 수집 게이트웨이 | Stock-exchange-BE, Hana-OmniLens-API | Planned |
| OCR 텍스트 추출, 위변조/딥페이크 1차 검증 | Hannah-Montana-AI 또는 별도 OCR service | Planned |
| 한국·홍콩 조세조약 케이스 판정 | Hana-OmniLens-API | Planned |
| 환급금 선지급 산정, 수수료, 사후 환수 상태 계약 | Hana-OmniLens-API | Planned |
