...
- Hannah-Montana-AI: 뉴스·공시 종목 매핑, 이벤트, 감성, 중요도 분석, 외국인 보유 시계열 예측 boundary 산출, 글로벌 피어 종목 매칭
...
- `POST /api/v1/korean-financial-terms/explain`는 뉴스/공시 본문에서 사용자가 누른 한국 금융 용어를 Hannah-Montana-AI 사전/RAG 엔진에 전달해 영어 해설을 반환한다. OmniLens는 설명 원문을 생성하지 않고, 검증 가능한 Hannah 응답만 TTL cache에 저장하며, 모든 클릭은 원문 사용자 키 대신 salted SHA-256 hash로 로그와 집계 테이블에 기록한다.
