...
- 뉴스 응답의 `modelVersion`은 Hannah의 이벤트, KF-DeBERTa 금융 감성, K-FNSPID 시장영향 모델 버전을 함께 보존한다. K-FNSPID 원천·시세 파일 생성과 학습은 Hannah 저장소 책임이며 OmniLens DB를 데이터셋 원천으로 사용하지 않는다.
