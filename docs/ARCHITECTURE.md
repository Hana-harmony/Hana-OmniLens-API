...
- `GET /api/v1/market/stocks/{stockCode}/global-peers`는 종목 master metadata를 Hannah-Montana-AI 글로벌 피어 매칭 모델에 전달해 외국인 투자자용 peer popup copy, 미국 상장 peer 목록, 속성별 `comparisons`, 국내 종목 자체의 4개 `keyStrengths`를 반환한다. AI 응답의 기준 종목 도메인과 모든 peer의 섹터·산업·주 사업 태그 정합성, dimension/icon key allowlist, 카드 개수를 provider 경계에서 검증하며, Hannah 장애 fallback은 근거 없는 비교·강점 카드를 만들지 않는다.
...
- 종목별 주기 수집은 OpenDART 공시를 뉴스보다 먼저 처리한다. `document.xml`의 `014` 등 오류 envelope는 본문으로 저장하지 않고 다음 주기에 재시도한다. V25는 알려진 `014` 오염 행을, V26은 영문 전문이 없는 과거 공시를 제거하며 `v2` Redis dedupe namespace로 즉시 공식 원문을 재수집한다.
- 종목 뉴스·공시 수집은 Hannah `FULL` 모드를 사용한다. 제목, What/Why/Impact, 영문 전문은 하나의 Qwen 분석 응답에서만 수용하고 전체 품질 gate를 통과한 이벤트만 DB·REST·WebSocket에 발행한다. 개별 번역 API나 규칙 기반 요약으로 후퇴하지 않는다.
